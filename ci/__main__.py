...
def main() -> bool:
    ...
    archive_name = args.archive_name or "crost"
    ...
    try:
        ...
        if dist_build:
            ...
            toolchain.Add_Define("CROST_BUILD")

            dist_include_dir, dist_lib_dir = StageLibraries(logger, dist_dir, None, [])


            crostToolchain = copy.copy(toolchain)
            crostToolchain.Add_Include_Directory(tools_dir / "crost")

            crostBuildMode = copy.copy(buildMode)
            crostBuildMode.host = HOST.HOSTED

            crost = Build_Executable(logger, crostToolchain, crostBuildMode, [], [], tools_dir / "crost", build_dir / "tools" / "crost", "crost")


            StageExecutables(logger, dist_dir, [crost])
